...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
		...
		$hlblock = Bitrix\Highloadblock\HighloadBlockTable::getById($arHlblock['ID'])->fetch(); //где ID - id highloadblock блока из которого будем получать данные
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
			$fieldEnumRes = CUserFieldEnum::GetList(
				array(),
				array("USER_FIELD_ID" => $fieldID)
			);

			while($arEnum = $fieldEnumRes->Fetch()){
				$result[] = $arEnum;
			}
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $rsAgents = $entity::GetList([
             ...
             */		
			"select" => array("ID", "UF_NAME", "UF_EMAIL", "UF_ACTIVITY", "UF_PHONE", "UF_ACTIVITY_TYPE", "UF_PHOTO"),
    ...
    public function clickStarAction($agentID)
    {
        ...
        $favouriteAgents = []; // массив ID элементов, которые пользователь добавил в избраное
		...
		if($favouriteAgents){
			...
			if(!in_array($agentID, $favouriteAgents)){
				$favouriteAgents[] = $agentID;
				CUserOptions::SetOption($category, $name, $favouriteAgents, false, $user_id);
				$result['action'] = 'success';
			} else {
				unset($favouriteAgents[array_search($agentID, $favouriteAgents)]);
				CUserOptions::SetOption($category, $name, $favouriteAgents, false, $user_id);
				$result['action'] = 'success';
			}

		} else {
			CUserOptions::SetOption($category, $name, array($agentID), false, $user_id);
			$result['action'] = 'success';
		}
